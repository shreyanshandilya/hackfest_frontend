...
const findDay = (x) => {
	...
	return days[Math.trunc((21+Math.trunc(x/80))/24)];
};

const findTime = (x) => {
	let time = (9+Math.trunc(x/80))%12;
	if(!time) time=12;
	time += ((x%80)/80)*0.6;
...
const findTimeSuffix = (x) => {
	return Math.trunc((9+Math.trunc(x/80))/12)%2==0 ? 'pm' : 'am';
};
...
const TimelineTrackItem = (props) => (
	<div className="schedule__timeline-track-item" style={{ left: props.left, width: props.width }}
		onClick={
			() => {
				props.changeModalContent(props.event_name, calculateTimeOfEvent(props.left, props.width),props.img);
				props.openModal();
			}
		}
	>
